...
void CMyApp::InitPointCloud()
{
	...
	{
		input_file >> x >> y >> z;
		vert.push_back({ glm::vec3(x,y,z), glm::vec3(0,1,0), glm::vec2(0,0) });
		input_length++;
	}
	input_length /= 3;
	if (input_length > 0) {
		std::cout << "Succcesfully read " << input_length << " number of points." << std::endl;
	}

	m_PointCloudBuffer.BufferData(vert);

	// geometria VAO-ban való regisztrálása
	m_PointCloudVao.Init(
		{
			// 0-ás attribútum "lényegében" glm::vec3-ak sorozata és az adatok az m_CubeVertexBuffer GPU pufferben vannak
			{ CreateAttribute<		0,						// attribútum: 0
									glm::vec3,				// CPU oldali adattípus amit a 0-ás attribútum meghatározására használtunk <- az eljárás a glm::vec3-ból kikövetkezteti, hogy 3 darab float-ból áll a 0-ás attribútum
									...
									sizeof(Vertex)			// stride: a következõ csúcspont ezen attribútuma hány byte-ra van az aktuálistól
								>, m_PointCloudBuffer },
			{ CreateAttribute<1, glm::vec3, (sizeof(glm::vec3)), sizeof(Vertex)>, m_PointCloudBuffer },
			{ CreateAttribute<2, glm::vec2, (2 * sizeof(glm::vec3)), sizeof(Vertex)>, m_PointCloudBuffer },
		}
	);
}
...
bool CMyApp::Init()
{
	...
	m_woodTexture.FromFile("assets/tex.jpg");
...
void CMyApp::Render()
{
	...
	glm::mat4 pointCloudWorld = glm::mat4(1.0f);
	m_program.SetUniform("MVP", viewProj * pointCloudWorld);
	m_program.SetUniform("world", pointCloudWorld);
	m_program.SetUniform("worldIT", glm::inverse(glm::transpose(pointCloudWorld)));

	m_program.SetUniform("fu", 1000.0f);
	m_program.SetUniform("fv", 1000.0f);
	m_program.SetUniform("u0", 320.0f);
	m_program.SetUniform("v0", 240.0f);

	glPointSize(5.0f);
	glDrawArrays(GL_POINTS, 0, input_length);
